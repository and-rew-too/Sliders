...
for i in range(0,301):
  ...
  while error > 0.0005:
    ...
    Vnew = Vint - ( -I + 0.044*15.6*15.6 - Io*np.exp((q*(Vint+I*Rs)/(k*T))) -((Vint+I*Rs)/Rsh)*b)  /  ((  (-Io*q)/(k*T) *np.exp((q*(Vint+I*Rs)/(k*T)))) - b - 3.7*((0.1*Vint+(I*Rs))/(Rsh*(1+0.1 * (1-(Vint+I*Rs)/(-0.7))**-4.7))  ))
    error = abs(Vnew-Vint)
